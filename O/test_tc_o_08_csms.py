"""
TC_O_08 - Get a Specific Display Message - Priority
...
Purpose:
    To verify if the CSMS is able to request specific priority messages from the charging station
...
async def test_tc_o_08():
    ...
    # Before: Set up a display message with a specific priority
    ...
    configured_priority = configured_message.get('priority')
    ...
    # Tool validation: priority must be the configured priority
    msg_priority = cp._get_display_messages_data['priority']
    assert msg_priority is not None, "priority must be present in GetDisplayMessagesRequest"

    # Tool validation: id and state should be omitted
    ...
    response = await cp.send_notify_display_messages(
        ...
        message_info=[{
            'id': configured_id,
            'priority': configured_priority or MessagePriorityEnumType.normal_cycle,
    ...
    logging.info("TC_O_08 completed successfully")
    start_task.cancel()
    await ws.close()
