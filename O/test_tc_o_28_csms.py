"""
TC_O_28 - Set Display Message - Specific transaction - Remove message after EndTime
Use case: O02 | Requirements: N/a
System under test: CSMS

Description:
    This test case describes how the CSMS can be requested to send a SetDisplayMessageRequest to the
...
Purpose:
    To verify if the CSMS is able to send the request with an endTime for a specific transaction
...
Tool validations:
    * Step 1: Message SetDisplayMessageRequest
      ...
      - message.startDateTime is <omitted>
      - message.endDateTime is <Configured endDateTime>
...
VALID_IDTOKEN_IDTOKEN = os.environ.get('VALID_IDTOKEN_IDTOKEN', 'TEST_TOKEN_1')
VALID_IDTOKEN_TYPE = os.environ.get('VALID_IDTOKEN_TYPE', IdTokenEnumType.iso14443)


@pytest.mark.asyncio
async def test_tc_o_28():
    """Set Display Message - Specific transaction - Remove message after EndTime."""
    cp_id = BASIC_AUTH_CP
    ...
    # Tool validation: message.startDateTime must be omitted
    msg_start_dt = message.get('start_date_time') or message.get('startDateTime')
    assert msg_start_dt is None, \
        f"message.startDateTime should be omitted, got {msg_start_dt}"

    # Tool validation: message.endDateTime must be present
    msg_end_dt = message.get('end_date_time') or message.get('endDateTime')
    assert msg_end_dt is not None, \
        "message.endDateTime must be present in SetDisplayMessageRequest"
    ...
    logging.info("TC_O_28 completed successfully")
    start_task.cancel()
    await ws.close()
