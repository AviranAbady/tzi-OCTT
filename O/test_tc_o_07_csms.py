"""
TC_O_07 - Get a Specific Display Message - Id
...
async def test_tc_o_07():
    ...
    # Tool validation: id must be the configured id
    msg_ids = cp._get_display_messages_data['id']
    assert msg_ids is not None, "id must be present in GetDisplayMessagesRequest"
    assert configured_id in msg_ids, \
        f"Expected id to contain {configured_id}, got {msg_ids}"

    # Tool validation: priority and state should be omitted
    ...
    logging.info("TC_O_07 completed successfully")
    start_task.cancel()
    await ws.close()
