"""
TC_O_18 - Set Display Message - NotSupportedState
...
Purpose:
    To verify if the CSMS is able to send a display message with a specific state, on which the
...
Main:
    ...
    2. The Test System responds with a SetDisplayMessageResponse with status NotSupportedState
...
async def test_tc_o_18():
    ...
    # Respond with NotSupportedState
    cp._set_display_message_response_status = DisplayMessageStatusEnumType.not_supported_state
    ...
    # CS responded with NotSupportedState (handled by on_set_display_message handler)

    logging.info("TC_O_18 completed successfully")
    start_task.cancel()
    await ws.close()
