"""
TC_F_11 - Trigger message - MeterValues - Specific EVSE
...
Purpose:
    To verify if the CSMS is able to trigger the Charging Station to send a MeterValuesRequest for a
    specific EVSE, using a TriggerMessageRequest.

Main:
    1. CSMS sends TriggerMessageRequest (requestedMessage=MeterValues, evse.id=<configured>)
    2. CS responds with TriggerMessageResponse (status=Accepted)
    3. CS sends MeterValuesRequest (evseId=<configured>, meterValue[0].sampledValue.context=Trigger)
...
@pytest.mark.asyncio
async def test_tc_f_11():
    """Trigger message - MeterValues - Specific EVSE."""
    cp_id = BASIC_AUTH_CP
    ...
    # Step 3-4: CS sends MeterValuesRequest with Trigger context
    ...
    logging.info("TC_F_11 completed successfully")
    start_task.cancel()
    await ws.close()
