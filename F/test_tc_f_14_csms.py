"""
TC_F_14 - Trigger message - TransactionEvent - All EVSE
...
Purpose:
    ...
    all EVSE, using a TriggerMessageRequest.

Before:
    Reusable State: EnergyTransferStarted

Main:
    1. CSMS sends TriggerMessageRequest (requestedMessage=TransactionEvent)
    2. CS responds with TriggerMessageResponse (status=Accepted)
    3. CS sends TransactionEventRequest (evse.id omitted, triggerReason=Trigger,
...
@pytest.mark.asyncio
async def test_tc_f_14():
    """Trigger message - TransactionEvent - All EVSE."""
    cp_id = BASIC_AUTH_CP
    ...
    # Step 3-4: CS sends TransactionEventRequest for every EVSE
    ...
    logging.info("TC_F_14 completed successfully")
    start_task.cancel()
    await ws.close()
