"""
TC_F_15 - Trigger message - LogStatusNotification - Idle
...
Main:
    1. CSMS sends TriggerMessageRequest (requestedMessage=LogStatusNotification)
    2. CS responds with TriggerMessageResponse (status=Accepted)
    3. CS sends LogStatusNotificationRequest (status=Idle)
    4. CSMS responds with LogStatusNotificationResponse

Tool validations:
    * Step 1: TriggerMessageRequest
      - requestedMessage must be LogStatusNotification
...
@pytest.mark.asyncio
async def test_tc_f_15():
    """Trigger message - LogStatusNotification - Idle."""
    cp_id = BASIC_AUTH_CP
    ...
    assert cp._trigger_message_data == MessageTriggerEnumType.log_status_notification or \
           cp._trigger_message_data == 'LogStatusNotification', \
        f"Expected requestedMessage=LogStatusNotification, got {cp._trigger_message_data}"

    # Step 3-4: CS sends LogStatusNotificationRequest (status=Idle)
    response = await cp.send_log_status_notification_request(status='Idle')
    assert response is not None

    logging.info("TC_F_15 completed successfully")
    start_task.cancel()
    await ws.close()
