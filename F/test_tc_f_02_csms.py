"""
TC_F_02 - Remote start transaction - Remote start first - AuthorizeRemoteStart is true
Use case: F02 | Requirements: F02.FR.01, F01.FR.01
...
F01.FR.01: Charging Station receives a RequestStartTransactionRequest AND ( AuthorizeRemoteStart = true AND idToken.type is NOT Central or NoAuthorization ) The Charging Station SHALL behave as if in response to a local action at the Charging Station to allow energy transfer after successful
    Precondition: Charging Station receives a RequestStartTransactionRequest AND (AuthorizeRemoteStart = true AND idToken.type is NOT Central or NoAuthorization)
...
Purpose:
    ...
    Charging Station receives a RequestStartTransactionRequest message (while AuthorizeRemoteStart is true),
    before the EV driver connects the EV and EVSE (within the connectionTimeout). The Charging Station has
    to authorize beforehand like a local action to start a transaction.
...
Main:
    ...
    3. CS sends AuthorizeRequest (idToken=<configured>)
    4. CSMS responds with AuthorizeResponse (idTokenInfo.status=Accepted)
    5. CS sends TransactionEventRequest (triggerReason=RemoteStart, remoteStartId=<generated>, eventType=Started)
    6. CSMS responds with TransactionEventResponse
    7. Execute Reusable State EnergyTransferStarted (Authorized, _EVConnected=false)
...
@pytest.mark.asyncio
async def test_tc_f_02():
    """Remote start transaction - Remote start first - AuthorizeRemoteStart is true."""
    cp_id = BASIC_AUTH_CP
    ...
    # Step 3-4: CS sends AuthorizeRequest
    ...
    assert authorize_response.id_token_info['status'] == 'Accepted', \
        f"Expected AuthorizeResponse.idTokenInfo.status=Accepted, got {authorize_response.id_token_info['status']}"

    # Step 5-6: CS sends TransactionEventRequest (Started, RemoteStart)
    ...
    # Step 7: Execute Reusable State EnergyTransferStarted (Authorized, _EVConnected=false)
    await energy_transfer_started(cp, evse_id=EVSE_ID, connector_id=CONNECTOR_ID,
                                  transaction_id=transaction_id)

    logging.info("TC_F_02 completed successfully")
    start_task.cancel()
    await ws.close()
