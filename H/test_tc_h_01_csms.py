"""
TC_H_01 - Reserve a specific EVSE - Accepted - Valid idToken
Use case: H01(S2), H03 | Requirements: N/a
...
Purpose:
    To verify if the CSMS is able to request the Charging Station to reserve a specific EVSE, until the
...
@pytest.mark.asyncio
async def test_tc_h_01():
    """Reserve a specific EVSE - Accepted - Valid idToken."""
    cp_id = BASIC_AUTH_CP
    ...
    # Wait for CSMS to send ReserveNowRequest
    ...
    # CS responded with Accepted (handled by on_reserve_now handler)

    # CS notifies CSMS about the status change - Reserved
    ...
    logging.info("TC_H_01 completed successfully")
    start_task.cancel()
    await ws.close()
