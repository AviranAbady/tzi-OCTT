"""
TC_K_36 - Get Charging Profile - EvseId > 0 + chargingProfilePurpose + stackLevel
...
Purpose:
    To verify if the CSMS is able to request charging profiles with a specific chargingProfilePurpose AND
    stackLevel installed on a specific EVSE and read in the reports at the OCPP specification.
...
async def test_tc_k_36():
    ...
    assert purpose is not None, "chargingProfilePurpose must be present"
    ...
    logging.info("TC_K_36 completed successfully")
    start_task.cancel()
    await ws.close()
