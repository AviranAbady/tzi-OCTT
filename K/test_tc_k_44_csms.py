"""
TC_K_44 - Get Composite Schedule - Charging Station
...
Purpose:
    To verify if the CSMS is able to calculate request a composite schedule from the Charging Station.

Main:
    1. The CSMS sends a GetCompositeScheduleRequest
    2. The OCTT responds with a GetCompositeScheduleResponse
       With status Accepted, schedule.evseId 0, schedule.duration 300
...
from ocpp.v201.enums import (
    Action, RegistrationStatusEnumType, ConnectorStatusEnumType, GenericStatusEnumType,
)
...
class SmartChargingMockCP(TziChargePoint):
    ...
    async def on_get_composite_schedule(self, duration, evse_id, charging_rate_unit=None, **kwargs):
        ...
        self._get_composite_schedule_data = {
            'duration': duration, 'evse_id': evse_id, 'charging_rate_unit': charging_rate_unit,
        }
        ...
        return call_result.GetCompositeSchedule(
            ...
            schedule={
                'evse_id': 0, 'duration': 300, 'schedule_start': now_iso(),
...
@pytest.mark.asyncio
async def test_tc_k_44():
    """Get Composite Schedule - Charging Station."""
    cp_id = BASIC_AUTH_CP
    ...
    assert req_data['evse_id'] == 0, f"Expected evseId=0, got {req_data['evse_id']}"
    ...
    logging.info("TC_K_44 completed successfully")
    start_task.cancel()
    await ws.close()
