"""
TC_K_34 - Get Charging Profile - EvseId > 0 + chargingLimitSource
...
Purpose:
    To verify if the CSMS is able to request charging profiles with a specific chargingLimitSource installed
...
Tool validations:
    * Step 1: Message GetChargingProfilesRequest
      ...
      - chargingProfile.chargingLimitSource <Configured chargingLimitSource>
"""
import asyncio
import logging
import os
...
async def test_tc_k_34():
    ...
    assert limit_source is not None, "chargingLimitSource must be present in criterion"
    ...
    logging.info("TC_K_34 completed successfully")
    start_task.cancel()
    await ws.close()
