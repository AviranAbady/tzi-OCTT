"""
TC_K_03 - Set Charging Profile - ChargingStationMaxProfile
Use case: K01 | Requirements: K01.FR.31, K01.FR.38
K01.FR.31: The startPeriod of the first chargingSchedulePeriod in a chargingSchedule SHALL always be 0.
K01.FR.38: When chargingProfilePurpose = ChargingStationMaxPr ofile chargingProfileKind SHALL NOT be Relative
    Precondition: When chargingProfilePurpose = ChargingStationMaxProfile
...
Purpose:
    To verify if the CSMS is able to send a ChargingStationMaxProfile charging profile as described at the
    OCPP specification.
...
Tool validations:
    * Step 1: Message SetChargingProfileRequest
      ...
      - chargingProfile.chargingProfilePurpose ChargingStationMaxProfile AND
      ...
      - chargingProfile.chargingSchedule.chargingSchedulePeriod.limit 8.0 or 8000.0 AND
...
CONFIGURED_NUMBER_PHASES = int(os.environ.get('CONFIGURED_NUMBER_PHASES', '3'))
...
async def test_tc_k_03():
    ...
    # chargingProfilePurpose must be ChargingStationMaxProfile
    purpose = profile.get('charging_profile_purpose') or profile.get('chargingProfilePurpose')
    assert purpose in ('ChargingStationMaxProfile', ChargingProfilePurposeEnumType.charging_station_max_profile), \
        f"Expected purpose=ChargingStationMaxProfile, got {purpose}"
    ...
    assert limit in (8.0, 8000.0), f"Expected limit=8.0 or 8000.0, got {limit}"
    ...
    logging.info("TC_K_03 completed successfully")
    start_task.cancel()
    await ws.close()
