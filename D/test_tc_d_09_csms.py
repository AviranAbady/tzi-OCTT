...
Configuration
    ...
    upon connection or when triggered. The CS (OCTT) is configured to respond with
    versionNumber=0 indicating no local authorization list is available.
...
Test Scenario
    ...
    3. CS responds with GetLocalListVersionResponse(versionNumber=0)

Tool validations
    No specific content validations on GetLocalListVersionRequest (no required fields).
    Post scenario: The CSMS must have correctly received versionNumber=0 from the CS.
"""
...
@pytest.mark.asyncio
async def test_tc_d_09():
    """Get Local List Version - No list available: CSMS requests local list version, CS responds with 0."""
    cp_id = BASIC_AUTH_CP
    ...
    logging.info(
        "GetLocalListVersionRequest received and responded with versionNumber=0 (no list available)"
    )
