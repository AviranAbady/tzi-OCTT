"""
TC_G_07 - Change Availability Connector - Operative to inoperative
...
Description:
    ...
    of the Connectors of one EVSE from Operative to Inoperative. A Connector is considered Operative in
    any status other than Faulted and Unavailable.
...
@pytest.mark.asyncio
async def test_tc_g_07():
    """Change Availability Connector - Operative to inoperative."""
    cp_id = BASIC_AUTH_CP
    ...
    # NotifyEventRequest with connector-level info
    event_data = [
        EventDataType(
    ...
    logging.info("TC_G_07 completed successfully")
    start_task.cancel()
    await ws.close()
