"""
TC_L_08 - Secure Firmware Update - InstallVerificationFailed
...
Description:
    ...
    downloads and verifies the firmware signature, starts installation, but the installation
    verification fails.

Purpose:
    To verify if the CSMS correctly handles an InstallVerificationFailed firmware status notification
...
Main:
    ...
    3. CS sends FirmwareStatusNotification progression:
       Downloading -> Downloaded -> SignatureVerified -> Installing -> InstallVerificationFailed
...
@pytest.mark.asyncio
async def test_tc_l_08():
    """Secure Firmware Update - InstallVerificationFailed."""
    cp_id = BASIC_AUTH_CP
    ...
    # FirmwareStatusNotification - InstallVerificationFailed
    resp = await cp.send_firmware_status_notification_request(
        FirmwareStatusEnumType.install_verification_failed
    )
    assert resp is not None

    logging.info("TC_L_08 completed successfully")
    start_task.cancel()
    await ws.close()
