"""
TC_L_20 - Publish Firmware - PublishFailed
Use case: L03 | Requirements: N/a
System under test: CSMS

Description:
    This test case covers the publish firmware process where the Charging Station downloads and
    verifies the checksum, but fails to publish the firmware.

Purpose:
    To verify if the CSMS correctly handles a PublishFailed publish firmware status notification
...
Main:
    ...
    3. CS sends PublishFirmwareStatusNotification progression:
       Downloading -> Downloaded -> ChecksumVerified -> PublishFailed
...
@pytest.mark.asyncio
async def test_tc_l_20():
    """Publish Firmware - PublishFailed."""
    cp_id = BASIC_AUTH_CP
    ...
    # PublishFirmwareStatusNotification - PublishFailed
    resp = await cp.send_publish_firmware_status_notification_request(
        status=PublishFirmwareStatusEnumType.publish_failed,
        request_id=request_id,
    )
    assert resp is not None

    logging.info("TC_L_20 completed successfully")
    start_task.cancel()
    await ws.close()
