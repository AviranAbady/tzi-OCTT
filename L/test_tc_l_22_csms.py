...
@pytest.mark.asyncio
async def test_tc_l_22():
    """Unpublish Firmware - NoFirmware."""
    cp_id = BASIC_AUTH_CP
    ...
    # Configure CS to respond with NoFirmware
    cp._unpublish_firmware_response_status = UnpublishFirmwareStatusEnumType.no_firmware
    ...
    # CS responded with NoFirmware (configured before start)
    logging.info(f"CS responded with NoFirmware for checksum: {cp._unpublish_firmware_data['checksum']}")

    logging.info("TC_L_22 completed successfully")
    start_task.cancel()
    await ws.close()
