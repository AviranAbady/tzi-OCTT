"""
TC_L_07 - Secure Firmware Update - DownloadFailed
...
Description:
    ...
    update request but the firmware download fails.

Purpose:
    To verify if the CSMS correctly handles a DownloadFailed firmware status notification from the
    Charging Station.

Main:
    1. CSMS sends UpdateFirmwareRequest
    2. CS responds with UpdateFirmwareResponse (status=Accepted)
    3. CS sends FirmwareStatusNotification: Downloading -> DownloadFailed
...
async def test_tc_l_07():
    ...
    # FirmwareStatusNotification - DownloadFailed
    resp = await cp.send_firmware_status_notification_request(
        FirmwareStatusEnumType.download_failed
    )
    assert resp is not None

    logging.info("TC_L_07 completed successfully")
    start_task.cancel()
    await ws.close()
