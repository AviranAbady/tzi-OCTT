"""
TC_N_50 - Alert Event - Periodic Triggered
Use case: N07 | Requirements: N/a
System under test: CSMS

Description:
    Charging Station sends NotifyEventRequest with eventData.trigger = Periodic.

Purpose:
    To test that CSMS returns a NotifyEventResponse when it receives
    a NotifyEventRequest with eventData.trigger set to Periodic.

Main:
    1. OCTT sends NotifyEventRequest with eventData.trigger = Periodic
...
async def test_tc_n_50():
    ...
    # Steps 1-2: Send NotifyEventRequest with trigger = Periodic
    event_data = [{
        'event_id': 1,
        'timestamp': now_iso(),
    ...
    logging.info("TC_N_50 completed successfully")
    start_task.cancel()
    await ws.close()
