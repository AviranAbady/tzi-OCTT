"""
TC_N_31 - Clear Customer Information - Clear and report + no data
...
Description:
    CSMS sends a message to the Charging Station to clear (and retrieve)
    IdToken customer information. The OCTT has no data but still sends
    NotifyCustomerInformation with minimal data.

Purpose:
    To test that CSMS supports sending a CustomerInformationRequest with
    report=true, clear=true and a valid idToken, and that the CSMS correctly
    handles a NotifyCustomerInformation with no meaningful data.
...
Main:
    ...
    3. OCTT sends NotifyCustomerInformationRequest (minimal data)
...
async def test_tc_n_31():
    ...
    logging.info("TC_N_31 step 1-2 completed: CustomerInformationResponse Accepted")

    # Step 3-4: OCTT sends NotifyCustomerInformationRequest (no data available)
    ...
    logging.info("TC_N_31 completed successfully")
    start_task.cancel()
    await ws.close()
