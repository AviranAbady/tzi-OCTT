"""
TC_N_48 - Alert Event - Variable monitoring on write only
Use case: N07 | Requirements: N/a
System under test: CSMS

Description:
    Charging Station sends NotifyEventRequest with eventData.actualValue
    empty (write-only variable).

Purpose:
    To test that CSMS returns a NotifyEventResponse when it receives
    a NotifyEventRequest with eventData.actualValue set to an empty string.

Main:
    1. OCTT sends NotifyEventRequest with eventData.actualValue empty
...
async def test_tc_n_48():
    ...
    # Steps 1-2: Send NotifyEventRequest with actualValue empty (write-only variable)
    event_data = [{
        'event_id': 1,
        'timestamp': now_iso(),
    ...
    logging.info("TC_N_48 completed successfully")
    start_task.cancel()
    await ws.close()
