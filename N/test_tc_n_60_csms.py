"""
TC_N_60 - Get Monitoring Report - with component criteria and list of components/variables
...
Description:
    ...
    list of components and variables. The request includes multiple componentVariable entries.

Purpose:
    To test that CSMS supports requesting a monitoring report filtered by both monitoringCriteria
    and a list of componentVariable entries, and that it handles an empty result set.

Main:
    1. CSMS sends GetMonitoringReportRequest with monitoringCriteria = DeltaMonitoring AND
       componentVariable[0].component.name = "ChargingStation", variable.name = "AvailabilityState",
       componentVariable[1].component.name = "EVSE", component.evse.id = <configured>, variable.name = "AvailabilityState"
...
def validate_component_variable_list(component_variable, evse_id):
    """Validate that componentVariable contains both ChargingStation/AvailabilityState
    and EVSE/AvailabilityState entries."""
    found_cs_avail = False
    ...
    for cv in component_variable:
        ...
        if comp.get('name') == 'ChargingStation' and var.get('name') == 'AvailabilityState':
            found_cs_avail = True
        if (comp.get('name') == 'EVSE'
                and evse.get('id') == evse_id
                and var.get('name') == 'AvailabilityState'):
            found_evse_avail = True

    assert found_cs_avail, \
        f"Expected componentVariable with component.name=ChargingStation, " \
        f"variable.name=AvailabilityState, got {component_variable}"
    assert found_evse_avail, \
        f"Expected componentVariable with component.name=EVSE, evse.id={evse_id}, " \
        f"variable.name=AvailabilityState, got {component_variable}"


@pytest.mark.asyncio
async def test_tc_n_60():
    """Get Monitoring Report - with component criteria and list of components/variables."""
    cp_id = BASIC_AUTH_CP
    ...
    # First request: DeltaMonitoring + ChargingStation&EVSE/AvailabilityState -> EmptyResultSet
    ...
    # (DeltaMonitoring + ChargingStation/AvailabilityState + EVSE/AvailabilityState)
    ...
    # Validate componentVariable list
    ...
    validate_component_variable_list(component_variable, CONFIGURED_EVSE_ID)

    logging.info("TC_N_60 step 1-2 completed: DeltaMonitoring + CS&EVSE/AvailabilityState -> EmptyResultSet")
    ...
    # Step 3-4: Wait for CSMS to send GetMonitoringReportRequest
    # (ThresholdMonitoring + ChargingStation/AvailabilityState + EVSE/AvailabilityState)
    await asyncio.wait_for(
        cp._received_get_monitoring_report.wait(),
        timeout=CSMS_ACTION_TIMEOUT,
    )
    ...
    logging.info("TC_N_60 completed successfully")
    start_task.cancel()
    await ws.close()
