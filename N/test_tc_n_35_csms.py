"""
TC_N_35 - Retrieve Log Information - Security Log - Success
Use case: N01 | Requirements: N/a
System under test: CSMS

Description:
    The CSMS can request a Charging Station to upload a file with security log
    information to a given location (URL). The Charging Station successfully
    uploads a log file and gives information about the status of the upload
    by sending status notifications to the CSMS.

Purpose:
    To verify if the CSMS is able to request a charging station to successfully
    upload a security log.

Main:
    1. CSMS sends GetLogRequest with logType = SecurityLog
...
@pytest.mark.asyncio
async def test_tc_n_35():
    """Retrieve Log Information - Security Log - Success."""
    cp_id = BASIC_AUTH_CP
    ...
    # Tool validation Step 1: logType = SecurityLog
    assert log_type == 'SecurityLog', \
        f"Expected logType=SecurityLog, got {log_type}"

    request_id = cp._get_log_data['request_id']
    assert request_id is not None, "GetLogRequest must contain a requestId"

    logging.info(f"TC_N_35 step 1-2 completed: GetLogResponse Accepted, requestId={request_id}")
    ...
    logging.info("TC_N_35 step 3-4 completed: LogStatusNotification Uploading")
    ...
    logging.info("TC_N_35 completed successfully")
    start_task.cancel()
    await ws.close()
