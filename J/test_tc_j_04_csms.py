"""
TC_J_04 - Clock-aligned Meter Values - Signed
Use case: J01 | Requirement: J01.FR.21
J01.FR.21: AlignedDataSignReadings is true The Charging Station SHALL retrieve signed meter values from components that support data signing and put them in the signedMeterValue field.
    Precondition: AlignedDataSignReadings is true
...
Purpose:
    ...
    when a transaction ends, with signed meter values.
...
Main:
    1. Execute Reusable State EVDisconnected
       ...
       - timestamp at clock-aligned interval
...
TX_ENDED_METER_VALUES_INTERVAL = int(os.environ.get('TX_ENDED_METER_VALUES_INTERVAL', '1'))
...
async def test_tc_j_04(connection):
    """Clock-aligned Meter Values - Signed (J01.FR.21).
    J01.FR.21: AlignedDataSignReadings is true The Charging Station SHALL retrieve signed meter values from components that support data signing and put them in the signedMeterValue field.
        Precondition: AlignedDataSignReadings is true
    """
    ...
    # TransactionEvent Ended with clock-aligned MeterValue timestamps and signed meter values
