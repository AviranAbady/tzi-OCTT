"""
TC_M_14 - Retrieve certificates from Charging Station - V2GRootCertificate
...
Purpose:
    To verify if the CSMS is able to retrieve the hashData from all V2GRootCertificate stored
...
@pytest.mark.asyncio
async def test_tc_m_14():
    """Retrieve certificates from Charging Station - V2GRootCertificate."""
    cp_id = BASIC_AUTH_CP
    ...
    if isinstance(cert_type, list):
        assert GetCertificateIdUseEnumType.v2g_root_certificate in cert_type, \
            f"Expected V2GRootCertificate in certificateType list, got {cert_type}"
    else:
        assert cert_type == GetCertificateIdUseEnumType.v2g_root_certificate, \
            f"Expected V2GRootCertificate, got {cert_type}"

    logging.info("TC_M_14 completed successfully")
    start_task.cancel()
    await ws.close()
