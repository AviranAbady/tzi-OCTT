"""
TC_E_02 - Start transaction options - EnergyTransfer
Use case: E01(S6) | Requirement: E01.FR.06
E01.FR.06: TxStartPoint contains: EnergyTransfer AND Energy flow starts AND No transaction has started yet on this EVSE The Charging Station SHALL start a transaction and send a TransactionEventRequest (eventType = Started) to the CSMS.
    Precondition: TxStartPoint contains: EnergyTransfer AND Energy flow starts AND No transaction has started yet on this EVSE
System under test: CSMS

Purpose: Verify the CSMS handles a Charging Station that starts a transaction when energy transfer
starts (chargingState=Charging in Started event).
...
async def test_tc_e_02(connection):
    """Start transaction options - EnergyTransfer (E01.FR.06).
    E01.FR.06: TxStartPoint contains: EnergyTransfer AND Energy flow starts AND No transaction has started yet on this EVSE The Charging Station SHALL start a transaction and send a TransactionEventRequest (eventType = Started) to the CSMS.
        Precondition: TxStartPoint contains: EnergyTransfer AND Energy flow starts AND No transaction has started yet on this EVSE
    """
    ...
    # Step 5-6: TransactionEvent Started / ChargingStateChanged / Charging
