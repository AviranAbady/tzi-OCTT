"""
TC_E_22 - Stop transaction options - EnergyTransfer stopped - SuspendedEV
...
transfer was stopped by the EV (chargingState=SuspendedEV, stoppedReason=StoppedByEV).
...
async def test_tc_e_22(connection):
    """Stop transaction options - EnergyTransfer stopped - SuspendedEV (E06.FR.07).
    E06.FR.07: TxStopPoint contains: EnergyTransfer AND Energy transfer stops The Charging Station SHALL stop the transaction and send a TransactionEventRequest (eventType = Ended) to the CSMS.
        Precondition: TxStopPoint contains: EnergyTransfer AND Energy transfer stops
    """
    ...
    # Step 1-2: TransactionEvent Ended / ChargingStateChanged / SuspendedEV / StoppedByEV
    end_event = TransactionEvent(
        ...
        transaction_info={
            ...
            'stopped_reason': StoppedReasonType.stopped_by_ev,
        },
    )
