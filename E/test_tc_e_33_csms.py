"""
TC_E_33 - Check Transaction status - Without transactionId - with message in queue
Use case: E14 | Requirements: E14.FR.06, E14.FR.07
...
E14.FR.07: The Charging Station receives a GetTransactionStatusRequest without a transactionId AND It has transaction-related messages to be delivered The Charging Station’s response SHALL have messagesInQueue = true.
    Precondition: The Charging Station receives a GetTransactionStatusRequest without a transactionId AND It has transaction-related messages to be delivered
System under test: CSMS

Purpose: Verify the CSMS can request the status of queued TransactionEventRequest messages by
sending GetTransactionStatusRequest without a transactionId. The CS responds that there are
messages queued.
...
2. CS waits TRANSACTION_DURATION seconds, then reconnects
3. CSMS sends GetTransactionStatusRequest (transactionId omitted)
4. CS responds with ongoingIndicator omitted, messagesInQueue=true
...
TRANSACTION_DURATION = int(os.environ.get('TRANSACTION_DURATION', '5'))
CSMS_ACTION_TIMEOUT = int(os.environ.get('CSMS_ACTION_TIMEOUT', '30'))


@pytest.mark.asyncio
async def test_tc_e_33():
    """Check Transaction status - without transactionId - with message in queue (E14.FR.06, E14.FR.07).
    E14.FR.06: The Charging Station receives a GetTransactionStatusRequest without a transactionId The Charging Station’s response SHALL NOT have ongoingIndicator set.
        Precondition: The Charging Station receives a GetTransactionStatusRequest without a transactionId
    E14.FR.07: The Charging Station receives a GetTransactionStatusRequest without a transactionId AND It has transaction-related messages to be delivered The Charging Station’s response SHALL have messagesInQueue = true.
        Precondition: The Charging Station receives a GetTransactionStatusRequest without a transactionId AND It has transaction-related messages to be delivered
    ...
    try:
        ...
        meter_value = MeterValueType(
            timestamp=now_iso(),
            sampled_value=[SampledValueType(value=2.5)],
        )
