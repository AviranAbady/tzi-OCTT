"""
TC_E_08 - Stop transaction options - EnergyTransfer stopped - StopAuthorized
Use case: E06(S6) | Requirement: E06.FR.07
E06.FR.07: TxStopPoint contains: EnergyTransfer AND Energy transfer stops The Charging Station SHALL stop the transaction and send a TransactionEventRequest (eventType = Ended) to the CSMS.
    Precondition: TxStopPoint contains: EnergyTransfer AND Energy transfer stops
System under test: CSMS

Purpose: Verify the CSMS handles a Charging Station that stops a transaction when the energy
transfer stopped normally (stop authorized, energy transfer stop configuration).

Before: Reusable State StopAuthorized
...
async def test_tc_e_08(connection):
    """Stop transaction options - EnergyTransfer stopped - StopAuthorized (E06.FR.07).
    E06.FR.07: TxStopPoint contains: EnergyTransfer AND Energy transfer stops The Charging Station SHALL stop the transaction and send a TransactionEventRequest (eventType = Ended) to the CSMS.
        Precondition: TxStopPoint contains: EnergyTransfer AND Energy transfer stops
    """
    ...
    # Step 1-2: TransactionEvent Ended / ChargingStateChanged / EVConnected / Local
