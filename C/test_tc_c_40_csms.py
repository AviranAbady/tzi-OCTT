...
Before (Preparations)
    ...
    Memory State:           Two valid idTokens with same GroupId are configured in the Local Authorization List
    Reusable State(s):      State is EVConnectedPreSession

Test Scenario
1. The OCTT sends a TransactionEventRequest with
    - triggerReason Authorized
    - idToken.idToken <Configured valid_idtoken_idtoken> which is in the Local Authorization List (with GroupId)
...
4. The OCTT sends a TransactionEventRequest with
    - triggerReason StopAuthorized
    - idToken.idToken <Configured valid_idtoken2_idtoken> (same GroupId, in Local Authorization List)
    - idToken.type <Configured valid_idtoken2_type>
    - eventType Updated

5. The CSMS responds with a TransactionEventResponse
    - idTokenInfo.status Accepted
    - idTokenInfo.groupIdToken.idToken <Configured groupIdToken>
...
Configuration
    VALID_ID_TOKEN / VALID_ID_TOKEN_TYPE:     first idToken with GroupId, in Local Authorization List
    VALID_ID_TOKEN_2 / VALID_ID_TOKEN_TYPE_2: second idToken with same GroupId, in Local Authorization List
...
@pytest.mark.parametrize("connection", [(BASIC_AUTH_CP, get_basic_auth_headers(BASIC_AUTH_CP, BASIC_AUTH_CP_PASSWORD))],
                         indirect=True)
async def test_tc_c_40(connection):
    ...
    # 1. TransactionEventRequest: Authorized with valid_idtoken (from Local Authorization List with GroupId)
    ...
    # 2. CSMS responds with TransactionEventResponse: Accepted + groupIdToken
    ...
    # 4. TransactionEventRequest: StopAuthorized with valid_idtoken2 (same GroupId, in Local Authorization List)
    ...
    # 5. CSMS responds with TransactionEventResponse: Accepted + groupIdToken
