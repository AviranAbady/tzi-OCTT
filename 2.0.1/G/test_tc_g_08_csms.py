"""
TC_G_08 - Change Availability Connector - Inoperative to operative
...
Description:
    ...
    of the Connectors from one EVSE from Inoperative to Operative. A Connector is considered Operative
    in any status other than Faulted and Unavailable.
...
Before:
    Memory State: Unavailable for <Configured connectorId>
...
Main:
    ...
    3. CS notifies CSMS (StatusNotificationRequest + NotifyEventRequest)
...
@pytest.mark.asyncio
async def test_tc_g_08():
    """Change Availability Connector - Inoperative to operative."""
    cp_id = BASIC_AUTH_CP
    ...
    # Before: Memory State is Unavailable for connector
    ...
    assert evse is not None, "Expected evse to be present"
    if isinstance(evse, dict):
        assert evse.get('id') == EVSE_ID, \
            f"Expected evse.id={EVSE_ID}, got {evse.get('id')}"
        assert evse.get('connector_id') == CONNECTOR_ID, \
            f"Expected evse.connectorId={CONNECTOR_ID}, got {evse.get('connector_id')}"

    # Step 3: CS notifies CSMS - Available
    ...
    logging.info("TC_G_08 completed successfully")
    start_task.cancel()
    await ws.close()
