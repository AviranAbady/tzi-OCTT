"""
TC_E_38 - Local start transaction - EV not ready
Use case: E03
System under test: CSMS

Purpose: Verify the CSMS handles a Charging Station that reports an EV is not ready to start the
energy transfer yet (chargingState=SuspendedEV after authorization and EV connection).

Before: Reusable State Authorized

Test sequence:
1. Execute Reusable State EVConnectedPreSession (EV connects)
2. CS sends TransactionEvent Updated / ChargingStateChanged / SuspendedEV (EV not ready)
...
async def test_tc_e_38(connection):
    """Local start transaction - EV not ready (E03)."""
    ...
    # Step 1: EVConnectedPreSession (EV connects after authorization)
    ...
    # Step 2-3: TransactionEvent Updated / ChargingStateChanged / SuspendedEV (EV not ready)
    suspended_event = TransactionEvent(
        event_type=TransactionEventType.updated,
        timestamp=now_iso(),
    ...
    response = await cp.send_transaction_event_request(suspended_event)
