"""
TC_M_18 - Retrieve certificates from Charging Station - All certificateTypes
...
Purpose:
    To verify if the CSMS is able to retrieve the hashData from all Root CA and V2GCertificateChain
    certificates stored at the Charging Station.
...
Main:
    ...
    2. The OCTT responds with a GetInstalledCertificateIdsResponse
       ...
       certificateHashDataChain contains the hashData of all certificates stored at the OCTT truststore.
...
@pytest.mark.asyncio
async def test_tc_m_18():
    """Retrieve certificates from Charging Station - All certificateTypes."""
    cp_id = BASIC_AUTH_CP
    ...
    # Return all certificate types in response
    ...
    cp._get_installed_certificate_ids_response_chain = [
        ...
        {
            'certificate_type': GetCertificateIdUseEnumType.manufacturer_root_certificate,
            'certificate_hash_data': {
                'hash_algorithm': HashAlgorithmEnumType.sha256,
        ...
        {
            'certificate_type': GetCertificateIdUseEnumType.v2g_root_certificate,
            'certificate_hash_data': {
                'hash_algorithm': HashAlgorithmEnumType.sha256,
                'issuer_name_hash': '99aabbcc' * 8,
                'issuer_key_hash': 'ddeeff00' * 8,
                'serial_number': '09101112',
            },
        },
        {
            ...
            'certificate_hash_data': {
                'hash_algorithm': HashAlgorithmEnumType.sha256,
                'issuer_name_hash': 'aabb1122' * 8,
                'issuer_key_hash': 'ccdd3344' * 8,
                'serial_number': '13141516',
            },
        },
    ]
    ...
    # Tool validation: certificateType is omitted
    cert_type = cp._get_installed_certificate_ids_data['certificate_type']
    assert cert_type is None, \
        f"Expected certificateType to be omitted (None), got {cert_type}"

    logging.info("TC_M_18 completed successfully")
    start_task.cancel()
    await ws.close()
