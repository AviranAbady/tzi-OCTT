"""
TC_O_17 - Set Display Message - NotSupportedPriority
...
Purpose:
    To verify if the CSMS is able to send a display message with a specific priority, on which the
    Charging station responds not supported, according to the DisplayMessage mechanism as described
    in the OCPP specification.

Main:
    1. The CSMS sends a SetDisplayMessageRequest
    2. The Test System responds with a SetDisplayMessageResponse with status NotSupportedPriority
...
async def test_tc_o_17():
    ...
    # Respond with NotSupportedPriority
    cp._set_display_message_response_status = DisplayMessageStatusEnumType.not_supported_priority
    ...
    # CS responded with NotSupportedPriority (handled by on_set_display_message handler)

    logging.info("TC_O_17 completed successfully")
    start_task.cancel()
    await ws.close()
