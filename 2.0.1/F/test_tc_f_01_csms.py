"""
TC_F_01 - Remote start transaction - Cable plugin first
Use case: F01 | Requirements: N/a
System under test: CSMS

Description:
    OCPP 2.x.x allows an EV driver to either first connect the EV and EVSE OR wait for/trigger a
    RequestStartTransactionRequest. Both sequences will result in being able to charge.

Purpose:
    To verify if the CSMS is able to handle a Charging Station that starts a charging session when the EV
...
Main:
    ...
    2. CS responds with RequestStartTransactionResponse (status=Accepted, transactionId=<generated>)
    3. CS sends TransactionEventRequest (triggerReason=RemoteStart, remoteStartId, eventType=Updated)
    4. CSMS responds with TransactionEventResponse
    5. Execute Reusable State EnergyTransferStarted (Authorized, _EVConnected=true)
...
async def test_tc_f_01():
    ...
    # Configure handler to include transactionId in response (spec: transactionId = <Generated>)
    cp._request_start_transaction_response_transaction_id = transaction_id
    ...
    # Step 3-4: CS sends TransactionEventRequest (Updated, RemoteStart)
    ...
    # Step 5: Execute Reusable State EnergyTransferStarted (Authorized, _EVConnected=true)
    # Part 2 only since EV is already connected
    ...
    logging.info("TC_F_01 completed successfully")
    start_task.cancel()
    await ws.close()
